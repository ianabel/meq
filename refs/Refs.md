...
is unported and was REMOVED on 2026-09-07 -- `git checkout 635aa3d -- attic/`
brings it back. These two are the seeds for bringing it
...
and the one MEQ's original free-boundary code implemented by hand, is
Lackner's: reduce the
...
CEDRES++ and by that original code alike. **It is important to be precise about
...
**A correction to an earlier version of this section.** It said the original code
...
That original code implements the third, not the first — which the old
...
What it gets wrong is narrower and more ordinary: Lackner's cost
...
| arXiv:2602.18130 (2026) | https://arxiv.org/abs/2602.18130 | Loibl, Teixeira, Toprak, Shishkina, Miao, Kiendl, Kummer & Marussig, *Comparative study of different quadrature methods for cut elements*. Thirteen open-source cut-quadrature codes identified, ten benchmarked from a common Matlab harness on 2D and 3D geometries with implicit and explicit boundaries. **The one structural conclusion**: the codes divide into those that TESSELLATE the boundary, which carry a geometric error and are limited to **second order** however many quadrature points they are given, and those that preserve a higher-order boundary, which alone reach higher rates. Algoim (dimension reduction by height functions) and BoSSS (moment fitting via the divergence theorem) are the two that reach their design order; Algoim is pre-asymptotic on coarse meshes and then reaches the lowest error of any. Also useful: raising the quadrature order does **not** help once the geometric approximation dominates, which is the same instrument-versus-answer separation this project keeps rediscovering, and the Strang & Fix epigraph in its footnote 49 -- *"It is not required that every polynomial which appears be integrated exactly"* -- is the licence for MEQ's answer below. **WHAT MEQ CONCLUDED FROM IT: do not adopt one.** The benchmarks are all Cartesian, MFEM's two cut backends (`MomentFittingIntRules` and the Algoim binding) are quadrilateral and hexahedral only -- the Algoim path aborts with *"supports only quads and hexes"* -- and MEQ's meshes are triangles. More decisively, `PlasmaEdgeConvergence` measures that an exact cut rule could not buy MEQ anything it does not already have: the plasma edge caps `psi*` at `j + 2.5` by APPROXIMATION, and a plain Gauss rule already attains that cap. See `CLAUDE.md`'s *The plasma edge caps the order*. Open access | CutElementQuadratureSurvey.pdf |
