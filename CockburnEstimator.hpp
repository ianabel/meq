...
class CockburnZhangNLEstimator : public mfem::ErrorEstimator
{
	protected:
		mfem::Mesh *mesh;
		mfem::FiniteElementSpace *q_space;
	...
	public:
	CockburnZhangNLEstimator( GridFunction &q, GridFunction &u, GridFunction &lambda_ref, Coefficient &kappa_ref, NLFunc rhs_ref )
		: q_sol( q ), u_sol( u ), lambda( lambda_ref ), kappa( kappa_ref ), F( rhs_ref ),valid( false )
	{
		q_space = q.FESpace();
		u_space = u.FESpace();
		m_space = lambda.FESpace();
	...
	double ComputeElementError( int i ) 
	{
		...
		for (int j=0; j < CellIntegrator.GetNPoints(); j++)
		{
			...
			double eta_1_tmp;
			double u_val = u_sol.GetValue( trans->ElementNo, ip );
			mfem::Vector pt( 2 );
			trans->Transform( ip, pt );
			// eta_1 = h_K^2 * || F_RHS + div q ||^2
			eta_1_tmp = ( F( pt, u_val )  + q_sol.GetDivergence( *trans ) );
			eta_1 += h_K * h_K * eta_1_tmp * eta_1_tmp * ip.weight;
