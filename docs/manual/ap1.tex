...
\item[\texttt{NewtonAbsoluteTolerance}] \emph{float, default $10^{-12}$,
	positive.}
\item[\texttt{LinearMaxIterations}] \emph{integer, default 1000, at least 1.}
\item[\texttt{LinearTolerance}] \emph{float, default $10^{-12}$, positive.}
\end{description}

\begin{quote}
	\textbf{The last two are parsed and validated but consumed by nothing.} The
	program drives a sparse direct solver. They are documented here because they
	are accepted; do not read a change in them as having had an effect.
\end{quote}
