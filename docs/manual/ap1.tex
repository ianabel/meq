...
\item[\texttt{PPrimeVariable}, \texttt{PPrimeFit}, \texttt{GGPrimeVariable},
	\texttt{GGPrimeFit}] are reserved for the same unwritten facility and are
	likewise refused. They appear in this path's accepted-key list because a key
	must be accepted before it can be given a message of its own; the generic
	unknown-key message would say that \texttt{PPrimeVariable} is not a key of
	\texttt{[source]}, which is the less useful of the two answers.
...
\item[Reserved keys] Five keys are reserved for reading several profiles out
	...
	\emph{refused} rather than ignored:
