...
namespace
{
	...
	{
		/// Set when this object owns the mesh; null when it borrows one that
		/// outlives it, as meq::AdaptiveDomain's does. `sub` is the handle
		/// either way.
		std::unique_ptr<mfem::SubMesh> owned;
	...
	/// **Omega_{p,h} THROUGH meq::AdaptiveDomain, BAND REFINED.** The instrument
	/// MEASUREMENTS.md M-84 is a sweep of, kept because the finding it produced
	/// is the one PE-0 most needs followed up: three band refinements take the
	/// transferred column's psi* from 501x the fitted column to 1.50x, so the
	/// transfer penalty M-83 identifies is removable. What it does NOT do is
	/// make the RATES pass -- a fixed band count is not a self-similar family,
	/// and every rate including the fitted column's falls about 0.5 below k+1.
	///
	/// Omega_{p,h} through meq::AdaptiveDomain, then
	/// refined @a bandRefinements times IN THE BAND ONLY: the elements of T_h
	/// owning a face of Gamma_{p,h} are marked, so step 3's second half pushes
	/// the refinement out into the elements Gamma_p cuts. The domain owns its
	/// computational mesh and refine() invalidates it, so it lives in a pool.
	PlasmaSubdomain makeBandRefinedSubdomain( int n, int bandRefinements )
	{
		static std::vector<std::unique_ptr<mfem::Mesh>> boxes;
		static std::vector<std::unique_ptr<meq::AdaptiveDomain>> pool;
		boxes.push_back( std::make_unique<mfem::Mesh>(
			mfem::Mesh::MakeCartesian2D( n, n, mfem::Element::TRIANGLE, false,
			                             pe0RMax - pe0RMin, pe0ZMax - pe0ZMin ) ) );
		boxes.back()->Transform( []( mfem::Vector const &in, mfem::Vector &out )
		{
			out( 0 ) = in( 0 ) + pe0RMin;
			out( 1 ) = in( 1 ) + pe0ZMin;
		} );

		pool.push_back( std::make_unique<meq::AdaptiveDomain>(
		...
		for ( int r = 0; r < bandRefinements; ++r )
		{
			...
			std::vector<char> touching(
				static_cast<std::size_t>( tee.GetNE() ), 0 );
			for ( int b = 0; b < tee.GetNBE(); ++b )
			{
				if ( tee.GetBdrAttribute( b ) != gammaAttribute )
					continue;
				int const e = tee.GetBdrElementFaceIndex( b );
				int e1 = -1, e2 = -1;
				tee.GetFaceElements( e, &e1, &e2 );
				if ( e1 >= 0 )
					touching[ static_cast<std::size_t>( e1 ) ] = 1;
			}
			mfem::Array<int> marked;
			for ( int e = 0; e < tee.GetNE(); ++e )
				if ( touching[ static_cast<std::size_t>( e ) ] )
					marked.Append( e );
			domain.refine( marked );
		}

		PlasmaSubdomain d;
		d.sub = &domain.computational();
		d.h = domain.largestElement();
		d.gammaP = domain.gammaHAttribute();
		d.gammaPMarker = domain.gammaHMarker();
		d.path = std::make_unique<mfem::VertexConePath>(
			*d.sub, d.gammaP, edgeLevelSet, 6.0*domain.largestElement() );
		return d;
	}
	...
	PlasmaSubdomain makePlasmaSubdomain( int n )
	{
		...
		domainAttr[ 0 ] = 1;
		d.owned = std::make_unique<mfem::SubMesh>(
	...
	///   ArtificialExactTraceOnGammaH
	///                 lambda evaluated on Gamma_{p,h} ITSELF, no transfer
	///                 anywhere. NOT a control, and the first draft of this file
	///                 had it down as one. **AND NOT A ROUTE EITHER, WHICH THE
	///                 NAME NOW SAYS**: it is unreachable by the coupled method,
	///                 because from PE-3 on lambda IS the interface unknown and
	///                 exists nowhere but on Gamma_p -- there is no function of
	///                 position to evaluate here. It was called `Fitted`, which
	///                 invited reading it as an implementation one could choose
	///                 instead of the transfer, and as a cost comparison against
	///                 a column nobody can run. It is a DIAGNOSTIC: the premise
	///                 measured with the transfer removed, and nothing else. On stage 5's OUTER boundary the
	...
	/// @param faceOrder the rule on each face of Gamma_{p,h} for the transferred
	///        datum; negative is MFEM's own default. It is a parameter because
	///        MEASUREMENTS.md M-83 is a sweep of it, and that sweep is the
	///        evidence that the quadrature is NOT what caps psi* on this route.
	EdgePoint measureOnPlasma( PlasmaEdge const &eq, int order, int n, Interface how,
	                           int faceOrder = -1, int bandRefinements = -1 )
	{
		PlasmaSubdomain d = ( bandRefinements >= 0 )
			? makeBandRefinedSubdomain( n, bandRefinements )
			: makePlasmaSubdomain( n );
		...
		solver.setExtensionQuadratureOrder( faceOrder );
		solver.setSource( sourceCoeff );
		if ( how == Interface::ArtificialExactTraceOnGammaH )
		{
	...
	///
	/// **AND THIS WINDOW IS THE BEST ONE AVAILABLE, WHICH IS A MEASUREMENT AND
	/// NOT A DEFAULT. THE SHORTFALL BELOW IS NOT THE INSTRUMENT.**
	///
	/// The obvious reading of the reds this case carries is that the sequence
	/// rate -- endpoint to endpoint over a span of 8 -- averages in a coarse
	/// mesh that is nowhere near asymptotic. It is a good reading: at h = 0.1
	/// Omega_{p,h} is TWENTY-SIX ELEMENTS, its dist/h reads 0.851 against 1.02
	/// to 1.33 for every other level, and the transferred column's psi* rate
	/// climbs monotonically pair by pair -- 1.441, 2.415, 2.836 at j = 0,
	/// k = 1 -- which is the shape of a pre-asymptotic head and not of a lost
	/// order.
	///
	/// IT IS ALSO WRONG. Measured by running the whole study at n = 128 as well
	/// and then reading every window out of the five levels --
	/// MEASUREMENTS.md M-81 carries the table and the evidence:
	///
	///     window                                     failures
	///     { 8, 16, 32, 64 }   <- this one                  17
	///     { 8, 16, 32, 64, 128 }                           20
	///     { 16, 32, 64, 128 }                              21
	///     k <= 2 { 16, 32, 64, 128 }, k = 3 { 16, 32, 64 } 22
	///     k <= 2 { 16, 32, 64, 128 }, k = 3 { 8, 16, 32, 64 } 18
	///
	/// Every window that drops the coarse mesh or adds the fine one is WORSE,
	/// and the reason is that the two columns run out of room at opposite ends.
	/// The FITTED column floors at n = 128 exactly as the paragraph above
	/// predicts -- L2( psi* ) 3.6e-14 at j = 0 and 1.9e-14 at j = 2, with the
	/// last pair rate collapsing to 4.764 and 3.147. The TRANSFERRED column
	/// stalls there for a different reason and nowhere near the floor: at
	/// j = 0, k = 3 its L2( q ) goes 2.34e-08 -> 7.01e-09, a last-pair rate of
	/// 1.739, and its psi* 5.48e-10 -> 7.84e-11 against the fitted column's
	/// 3.6e-14 -- so the transfer's own error is two to three ORDERS OF
	/// MAGNITUDE above the floor and is what psi* is measuring by then. At that
	/// point psi* has stopped buying anything: 7.84e-11 against psi_h's
	/// 9.54e-11 on the same row.
	///
	/// So the two halves of what this case reports are different findings, and
	/// the window cannot separate them because refining discriminates AGAINST
	/// both. The fitted column's per-pair rates do reach k+1 and k+2 at the
	/// fine end -- 2.907 and 3.909 at j = 1, k = 2 -- so the staircase costs a
	/// slowly vanishing perturbation rather than an order, and what fixes the
	/// REPORTED rate is a Gamma_{p,h} that is not a staircase, which is the
	/// meq::AdaptiveDomain lever the assertions below name. The transferred
	/// column's cap is the transfer itself and no geometry on the plasma side
	/// touches it.
	...
	std::vector<EdgePoint> pe0Study( PlasmaEdge const &eq, int order,
	                                 ...
	                                 int bandRefinements = -1 )
	{
		std::vector<EdgePoint> points;
		points.reserve( pe0Meshes.size() );
		for ( int n : pe0Meshes )
			points.push_back( measureOnPlasma( eq, order, n, how, -1,
			                                   bandRefinements ) );
		return points;
	}
	...
	struct SequenceRates
	{
		double psi, flux, star;
		/// The points themselves, so a caller can compare the two routes
		/// mesh by mesh rather than only rate by rate. MEASUREMENTS.md M-83.
		std::vector<EdgePoint> points;
	};
	...
	SequenceRates runPe0Sequence( PlasmaEdge const &eq, int k, Interface how,
	                              char const *what, int bandRefinements = -1 )
	{
		std::printf( "    %s:\n", what );
		...
		/*
		 * psi AND q ARE REPORTED AND NOT GATED ON A RATE HERE, AND THE REASON IS
		 * MEASURED RATHER THAN A CONCESSION.
		 *
		 * Both are INTERIOR dominated on this geometry -- MEASUREMENTS.md M-85
		 * measures L2( psi ) saturating at one band refinement, 2.329e-05,
		 * 2.311e-05, 2.310e-05, so nothing done at the interface moves it. What
		 * a rate gate on them scores is therefore the background discretisation,
		 * and `theSamePlasmaOnAFittedDomainIsCleanAtEveryVanishingOrder` already
		 * scores exactly that on a FITTED rectangle at a TIGHTER slack, k+1 less
		 * 0.10 against this file's 0.15, with no staircase and no transfer to
		 * confound it. Two gates on one claim, and the cleaner one is the other.
		 *
		 * AND THE GATE HERE WAS NOT MEASURING WHAT IT LOOKED LIKE. M-85: band 0
		 * gives psi a rate of 1.997 and one band refinement gives 1.515 -- while
		 * every error FALLS. The band removes the geometric error, whose benefit
		 * is 3.2x at n = 8 and 1.2x at n = 64, so it decays faster than psi's own
		 * O( h^(k+1) ); taking it away lowers the coarse end and the measured
		 * rate with it. So the 1.997 clears k+1 with help from a higher-order
		 * coarse-end term, and a gate that PASSES on the worse geometry and FAILS
		 * on the better one is scoring the contamination rather than the method.
		 *
		 * What survives is the assertion that cannot be gamed that way: the error
		 * FALLS at every refinement, checked pair by pair above. psi* keeps its
		 * rate gate, because psi* is what the interface treatment actually moves.
		 */
...
BOOST_AUTO_TEST_CASE( theUncutPlasmaKeepsItsOrderAtEveryVanishingOrder )
{
	...
	double const cutStar[ 3 ][ 3 ] = { { 1.89, 1.95, 1.70 },
	                                   { 3.00, 2.87, 2.68 },
	                                   { 3.00, 3.99, 3.88 } };
	SequenceRates artificial[ 3 ][ 3 ], transferred[ 3 ][ 3 ];
	...
	for ( int j = 0; j <= 2; ++j )
	{
		...
		for ( int k = 1; k <= 3; ++k )
		{
			...
			artificial[ j ][ k - 1 ] =
				runPe0Sequence( eq, k, Interface::ArtificialExactTraceOnGammaH,
			...
			/*
			 * AND THE SAME TRANSFER ON THE GEOMETRY THE METHOD SHOULD ACTUALLY
			 * USE: ONE BAND REFINEMENT. This is the gated one.
			 *
			 * meq::AdaptiveDomain::refine(), marking the elements of T_h that own
			 * a face of Gamma_{p,h}, grows T_h TOWARDS Gamma_p and takes dist/h
			 * from 1.256 to 0.471. MEASUREMENTS.md M-85: psi* improves 7x / 14x /
			 * 6.6x at k = 1, 2, 3 for 1.43x the elements, and its RATE improves
			 * with it -- 2.230 to 2.694 and 3.161 to 3.694. Going further pays
			 * less and returns worse rates.
			 *
			 * WHY THE GATE BELONGS HERE AND NOT ON THE BAND 0 ROW. From PE-3 on,
			 * lambda IS the interface unknown and lives only on Gamma_p, so the
			 * transfer is the method rather than one implementation of it -- the
			 * artificial column cannot be chosen instead. The row that should
			 * carry an acceptance is therefore the transfer on the geometry the
			 * method would be run on, and the band 0 transfer stays as the
			 * measurement of what the amplification costs.
			 */
			SequenceRates const banded =
				runPe0Sequence( eq, k, Interface::Transferred,
				                "lambda on Gamma_p, transferred, ONE BAND "
				                "REFINEMENT -- the geometry the method should use",
				                1 );

			/*
			 * THE AMPLIFICATION ITSELF, REPORTED AND DELIBERATELY NOT ASSERTED.
			 *
			 * This is the quantity M-83 identifies as what the transfer actually
			 * costs, and the quantity meq::AdaptiveDomain would move if the
			 * mechanism is right -- so printing it is what makes that experiment
			 * measurable in advance rather than after the fact.
			 *
			 * A CEILING ON ITS GROWTH WAS PROPOSED AND MEASURED OUT. Since the
			 * amplification is ( 1 + 2 dist/h )^k and assumption P.1 bounds
			 * dist/h, the ratio ought to PLATEAU rather than grow, and over the
			 * last refinement most rows oblige: 1.05 at j = 0, k = 1 and 2, 1.08
			 * to 1.17 across j = 1. But j = 2, k = 2 grows by 2.40 -- its
			 * transferred pair rates read 3.500, 3.520, 2.693, 2.037 while the
			 * fitted column is clean at 3.97 -- so the ratio inherits this route's
			 * own intermittency, which the header records on a geometry with no
			 * corner anywhere. Any ceiling loose enough to pass that row is too
			 * loose to catch a transfer leaving its regime, which is the only
			 * thing such a gate would be for.
			 */
			{
				std::printf( "    psi* amplification, transferred/fitted, by mesh:" );
				std::vector<EdgePoint> const &t = transferred[ j ][ k - 1 ].points;
				std::vector<EdgePoint> const &f = artificial[ j ][ k - 1 ].points;
				for ( std::size_t i = 0; i < t.size() && i < f.size(); ++i )
					std::printf( " %.1f", t[ i ].star/f[ i ].star );
				std::printf( "\n" );
				std::fflush( stdout );
			}

			// PE-0's stated acceptance, on both routes.
			BOOST_TEST( artificial[ j ][ k - 1 ].star >= k + 2.0 - pe0SequenceSlack,
			            "j = " << j << ", k = " << k << ": psi* converged at "
			            << artificial[ j ][ k - 1 ].star << " on the UNCUT plasma "
			            ...
			            "re-entrant corners, 225 and 270 degrees on this simplicial "
			            "background and MEASUREMENTS.md M-84 for the distribution "
			            "-- the companion "
			            "mesh of GS-2 section 3.3, meq::AdaptiveDomain, is the lever "
			            "this tree already has" );
			/*
			 * k+1 ACROSS THE SEQUENCE, AND THE ORDER IS NOT WHAT IS LOST.
			 *
			 * THE RATE IS INTACT. Read pair by pair at the fine end the
			 * transferred psi* gives 2.84 / 3.87 / 4.79 at n = 32 -> 64 against
			 * k+2 of 3 / 4 / 5. What the transfer costs is a CONSTANT, not an
			 * order, and the constant is large: at the finest mesh of this
			 * sequence psi* on this route is 30x the fitted column at k = 1, 93x
			 * at k = 2 and 555x at k = 3. A rate read endpoint to endpoint over a
			 * finite range of h therefore reads about k+1, because the error
			 * starts one to two orders higher and is still coming down.
			 *
			 * WHERE THE CONSTANT COMES FROM. The lifting evaluates E_h -- the
			 * element's OWN degree-k polynomial -- OUTSIDE its element, along a
			 * path of length d ~ 1.3 h. Extrapolating a degree-k polynomial a
			 * fixed multiple of its own support amplifies its error by a
			 * Chebyshev/Markov factor that explodes in k, and the measured ratio
			 * tracks BOTH variables: monotone in dist/h at fixed k, explosive in
			 * k at fixed dist/h. MEASUREMENTS.md M-83 has the table, and the
			 * comparison T_k( 1 + 2 d/h ) at d/h = 1.26 reading 3.5 / 23.8 /
			 * 163.9 -- the same shape and the same order of magnitude.
			 *
			 * SO IT IS NOT THE QUADRATURE, WHICH WAS THE FIRST READING AND IS
			 * MEASURED WRONG. The outer integral over each face of Gamma_{p,h} is
			 * genuinely inexact -- the foot map a( x ) is not polynomial in x and
			 * MFEM's header says so -- but setExtensionQuadratureOrder() converges
			 * it by order 12, and converging it leaves psi* at 4.335e-10 against
			 * the fitted column's 9.866e-13. It does buy L2( q ) a factor of 2.7
			 * at k = 3, which is worth having and is a separate finding.
			 *
			 * AND IT IS NOT "no geometry on the plasma side touches it", WHICH
			 * THIS COMMENT USED TO SAY. The constant is set by dist/h, and dist/h
			 * is exactly what GS-2 section 3.3's companion mesh exists to control
			 * -- meq::AdaptiveDomain, the same lever the fitted column's
			 * assertion names for an unrelated reason. Untested, and it is the
			 * experiment to run next.
			 *
			 * WHY THE GATE IS STILL k+1. It is the only floor that holds over the
			 * range this sequence spans, and it is ExtensionConvergence's own
			 * per-pair floor applied across the sequence: psi* is never worse than
			 * psi_h's own design rate. A floor chosen to sit clear of a working
			 * transfer and a broken one rather than an estimate of the true order,
			 * which is exactly how that file argues its 1.5 and 1.0. k+1.5, the
			 * obvious number, fails seven of these nine rows.
			 *
			 * WHAT THIS DOES NOT DO IS MAKE THE ROUTE GREEN: j = 1, k = 2 still
			 * fails at 2.615, dragged under by a first pair of 1.220 that no other
			 * row has. That row is a finding and is left visible.
			 */
			BOOST_TEST( banded.star >= k + 1.0 - pe0SequenceSlack,
			            "j = " << j << ", k = " << k << ": psi* converged at "
			            << banded.star << " with lambda transferred from Gamma_p on "
			            "ONE BAND REFINEMENT -- the geometry the coupled method "
			            "should run on -- against " << transferred[ j ][ k - 1 ].star
			            << " unrefined and " << artificial[ j ][ k - 1 ].star
			            << " with lambda on Gamma_{p,h}. Short of psi_h's own design "
			            "rate k+1 = " << k + 1 << ", which is this route's acceptance "
			            "and not k+2: MEASUREMENTS.md M-83 measures the transfer "
			            "costing a CONSTANT and not an order -- the pair rate at the "
			            "fine end IS k+2 -- so the sequence rate reads k+1 over this "
			            "range, and a floor there is what separates a working "
			            "transfer from a broken one. Below it the post-processing is "
			            "worse than the field it post-processes" );
		}
	}
	...
	for ( int k = 2; k <= 3; ++k )
		for ( SequenceRates const &route : { artificial[ 0 ][ k - 1 ],
		                                     transferred[ 0 ][ k - 1 ] } )
	...
	for ( int j = 0; j <= 2; ++j )
		std::printf( "     %d   %7.3f %7.3f %7.3f %7.3f %7.3f %7.3f %7.2f %7.2f %7.2f\n",
		             j,
		             artificial[ j ][ 0 ].star, artificial[ j ][ 1 ].star, artificial[ j ][ 2 ].star,
	...
	for ( int j = 0; j <= 2; ++j )
		std::printf( "     %d   %7.3f %7.3f %7.3f %7.3f %7.3f %7.3f %7.2f %7.2f %7.2f\n",
		             j,
		             artificial[ j ][ 0 ].psi, artificial[ j ][ 1 ].psi, artificial[ j ][ 2 ].psi,
	...
	for ( int k = 1; k <= 3; ++k )
	{
		double artificialStar[ 3 ], transferredStar[ 3 ], artificialPsi[ 3 ];
		for ( int j = 0; j <= 2; ++j )
		{
			artificialStar[ j ]  = artificial[ j ][ k - 1 ].star;
			transferredStar[ j ] = transferred[ j ][ k - 1 ].star;
			artificialPsi[ j ]   = artificial[ j ][ k - 1 ].psi;
		}
		std::printf( "     k = %d: spread over j -- psi* %.3f on Gamma_{p,h} and "
		             "%.3f transferred, psi %.3f  (the cut mesh's psi* spread is "
		             "%.2f)\n", k, spreadOverJ( artificialStar ),
		             spreadOverJ( transferredStar ), spreadOverJ( artificialPsi ),
		             cutStar[ 2 ][ k - 1 ] - cutStar[ 0 ][ k - 1 ] );

		BOOST_TEST( spreadOverJ( artificialStar ) < 0.4,
		            "psi*'s rate at k = " << k << " moved by "
		            << spreadOverJ( artificialStar ) << " between j = 0 and j = 2 on the "
		...
		BOOST_TEST( spreadOverJ( artificialPsi ) < 0.4,
		            "psi_h's rate at k = " << k << " moved by "
		            << spreadOverJ( artificialPsi ) << " between j = 0 and j = 2 on the "
