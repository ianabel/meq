...
| `FREE-BOUNDARY-PLAN.md` | item 8 — **the staged ladder is CLOSED**: FB-A to FB-7 done or answered, FB-6 met, and §10's XP-0 to XP-4 all met (M-82, M-86, M-87). The conductor model went to `COIL-SUBTRACTION-PLAN.md`. Its new **§12 collects the three claims that were still marked unmeasured** as FB-R, FB-S and FB-T, each with the experiment that settles it. **FB-R is now measured** → M-132: the initial `ψ_bnd` selects among discrete equilibria and **refinement does not merge them** — a factor of 6.3 in `ψ_bnd` at 43,026 elements. Suite 7/7 |
