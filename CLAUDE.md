...
| **`mfem::NormalTraceJumpIntegrator` carries unguarded scratch, and the loop upstream is threading REACHES IT** | `Vector face_shape, normal, shape1_n, shape2_n` and two `DenseMatrix` as plain members, no `#ifndef MFEM_THREAD_SAFE` (`fem/bilininteg.hpp:4872`). MEQ installs exactly one, as the object handed to `EnableHybridization()`, and `DarcyForm::ReconstructFluxAndPot()` takes it per face inside its element loop as `c_bfi`. **Live the moment `Reconstruct()` is threaded, and MEQ will not be refused** — `SetIntegratorsThreadSafe()` is already called, so the promise covers a loop whose audit was never taken. Filed as `../mfem-hdg-dev/doc/HDG-RECONSTRUCT-THREADING-FROM-MEQ.md` while the work is in progress. It is in `fem/` rather than `fem/darcy/`, which is plausibly why it was not among the eight upstream guarded. `THREADING-PLAN.md`'s addendum is MEQ's side |
...
| **`Reconstruct()` aborts under `Device("debug")`, and MEQ's diagnosis of WHY is withdrawn** | `debug`-only, and the last link of M-130's chain. The abort is real and reproducible — `Vector b_ze( b_z.GetData() + e*nd_ut, nd_ut )` at `darcyhybridization.cpp:10164` with `e == 0` aliases `b_z`'s base pointer exactly, and the next element's `b_z = 0.` aborts. **The mechanism MEQ filed for it is wrong.** `Memory<T>::Wrap( ptr, n, own = false )` does set `flags = VALID_HOST` alone and skip `Register_`, and `h_mt` does take `GetHostMemoryType()` regardless, so `std_delete` is false and `MemoryManager::Delete_` **is** called — but `Delete_` opens with `if (!mm.exists || !registered) { return; }` (`general/mem_manager.cpp:996`, in the tree `../mfem/install` is built from), so it evicts nothing. Both candidate fixes are therefore no-ops and upstream has withdrawn the one it had written, having built it and found the regression case passes with the change reverted. `../mfem-hdg-dev/doc/HDG-RECONSTRUCT-TOTAL-FLUX-FROM-MEQ.md` carries the correction at its head. **Open, and now open on the question of where the eviction comes from** — the next instrument is a breakpoint on `Delete_` conditioned on `b_z`'s base, reading `flags`. **The transferable part**: every link of that chain was checked except the last, and *a mechanism that explains a symptom is not the mechanism that causes it* |
