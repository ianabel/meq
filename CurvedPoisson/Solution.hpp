...
class Solution {
	...
	public:
		...
		// Takes an old solution's u* variable, and 
		// projects that into the current solutions's u
		// must be on the same mesh
		void ProjectUStar( Solution const& old_soln )
		{
			u_variable.ProjectGridFunction( old_soln.u_star_variable );
			hasUStar = false;
			u_star.Destroy();
		}
